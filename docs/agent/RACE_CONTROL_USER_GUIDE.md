...
6. The open Schedule tab refreshes automatically every 60 seconds while it is visible and refreshes again when it regains focus. The **Last sync** text records the latest successful read. Use the refresh icon after a Calendar change when you want an immediate read.
